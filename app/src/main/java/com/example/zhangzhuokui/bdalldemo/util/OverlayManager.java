 ...
 * </p>
 */
public abstract class OverlayManager implements OnMarkerClickListener, OnPolylineClickListener, BaiduMap.OnMapClickListener {
     ...
     * @param baiduMap
     */
    public OverlayManager(BaiduMap baiduMap, MapView mapView) {
        mBaiduMap = baiduMap;
        if (mapView != null) {
            mMapView = mapView;
//            mMapView.showScaleControl(false);
     ...
     * 将所有Overlay 添加到地图上
     */
    public final void addToMap(boolean isClear) {
        if (mBaiduMap == null) {
            return;
        }
...
//        mOverlayList.clear();
     ...
     * 
     */
    public void zoomToSpan(float zoom) {
        if (mBaiduMap == null) {
            return;
        }
        if (mOverlayList.size() > 0) {
            ...
            MapStatusUpdate u = MapStatusUpdateFactory.zoomTo(zoom);//设置缩放到MapStatusUpdate对象中
...
//            mBaiduMap.animateMapStatus(u);
    ...
    //单点
    public void zoomToSpan(LatLng point, float zoom) {
        if (mBaiduMap == null) {
            return;
        }
