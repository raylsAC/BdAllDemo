...
public class LabelView extends View {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mCornerSize = ta.getDimension(R.styleable.LabelView_lv_corner, dp2px(10));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mFillTriangle) {
            ...
            if (mGravity == (Gravity.TOP | Gravity.LEFT)) {
                mPath.reset();
                mPath.moveTo(0, mCornerSize);
                mPath.lineTo(0, size);
                mPath.lineTo(size, 0);
                mPath.lineTo(mCornerSize, 0);

                mPath.close();
                canvas.drawPath(mPath, mBackgroundPaint);

                float radiu = (float) (mCornerSize * 2);
                RectF mRectF = new RectF(0, 0, radiu, radiu);
                canvas.drawArc(mRectF, -90, -90, true, mBackgroundPaint);
