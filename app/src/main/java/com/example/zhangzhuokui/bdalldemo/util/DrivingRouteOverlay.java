    ...
    public final List<OverlayOptions> getOverlayOptions() {
        ...
        if (mRouteLine.getAllStep() != null
            ...
            for (DrivingRouteLine.DrivingStep step : mRouteLine.getAllStep()) {
                ...
                int a =	step.getDirection();//返回该路段起点方向值
                RouteNode node = step.getEntrance();//路段入口信息
                String	instructionsEnter = step.getEntranceInstructions();//路段入口的指示信息
                RouteNode	nodeExit = step.getExit();//路段出口信息
                String	instructionsExit = step.getExitInstructions();//路段出口指示信息
                String	insteuctions =  step.getInstructions();//路段总体指示信息
                int[] trafficList =	step.getTrafficList();//获取路况数据数组，个数为wayPoints个数-1
...
//            }
            ...
            PolylineOptions option = new PolylineOptions()
                                        ...
                                        .zIndex(0);
            if (isDotLine) {
                option.customTextureList(getCustomTextureList());//设置虚线自定义纹理
            }
